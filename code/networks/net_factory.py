...
from networks.vision_mamba import MambaUnet as mambaunet
...
def net_factory(net_type="unet", in_chns=1, class_num=4):
    ...
    elif net_type in ["ViT_Seg", "swinunet", "SwinUNet"]:
        net = ViT_seg(config, img_size=args.patch_size,
                    num_classes=args.num_classes).cuda()
    elif net_type == "mambaunet":
        net = mambaunet(config, img_size=args.patch_size, num_classes=args.num_classes).cuda()
    elif net_type == "pnet":
